...
var uuid = __webpack_require__(/*! uuid */ "./node_modules/uuid/dist/esm-browser/index.js").v4; // sessions is sid to username.


var sessions = {}; // tasks is username to task info.

var tasks = {};

var getSessionInfo = function getSessionInfo(sid) {
  return sessions[sid];
};

var getTasks = function getTasks(sid) {
  var sessionInfo = getSessionInfo(sid);
...
var findTaskByName = function findTaskByName(todos, taskname) {
...
var deleteSession = function deleteSession(sid) {
  delete sessions[sid];
};

var addNewTask = function addNewTask(sid, taskname) {
  var tasksInfo = getTasks(sid);
...
var deleteTask = function deleteTask(sid, taskid) {
  var tasksInfo = getTasks(sid);
  tasksInfo.todos = tasksInfo.todos.filter(function (todo) {
    return todo.taskId != taskid;
  });
  return tasksInfo;
};

var toggleDone = function toggleDone(sid, taskid) {
  var tasksInfo = getTasks(sid);
  var index = tasksInfo.todos.findIndex(function (todo) {
    return todo.taskId == taskid;
  });
...
/*!*************************!*\
  ...
  \*************************/
/*! namespace exports */
/*! export addTask [provided] [no usage info] [missing usage info prevents renaming] */
...
/*! export getTasks [provided] [no usage info] [missing usage info prevents renaming] */
...
/*! export toggleDone [provided] [no usage info] [missing usage info prevents renaming] */
...
var addTask = function addTask(taskname) {
  return fetch('/task', {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json'
...
var deleteTask = function deleteTask(taskid) {
  return fetch("/task/".concat(taskid), {
    method: 'DELETE',
    headers: {
      'Content-Type': 'application/json'
    }
...
var toggleDone = function toggleDone(taskid) {
  return fetch("/task/".concat(taskid), {
    method: 'PUT',
    headers: {
      'Content-Type': 'application/json'
...
"use strict";
__webpack_require__.r(__webpack_exports__);
/* harmony import */ var _session__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! ../session */ "./session.js");
...
(function iife() {
  ...
  addAbilityToLogout();
  (0,_services__WEBPACK_IMPORTED_MODULE_1__.checkLoginStatus)().then(function (tasksInfo) {
  ...
  function prepareContentPage(tasksInfo) {
    ...
    updateTasksPeriodically(); //renderTodoList(tasksInfo);
  ...
  function addAbilityToLogin() {
    var loginButtonEl = document.querySelector('#todo-app .login button').addEventListener('click', function () {
      var username = document.querySelector('#todo-app input').value;
      (0,_services__WEBPACK_IMPORTED_MODULE_1__.performLogin)(username).then(function (tasksInfo) {
        prepareContentPage(tasksInfo);
      })["catch"](function (errors) {
        showLogin();
        renderErrorStatus(errors);
  ...
  function renderTodoList(tasksInfo) {
    ...
    var todosHtml = tasksInfo.todos.map(function (todo) {
      var done = todo.done ? 'done' : '';
      return "<li class=\"todo\">\n            <button class='delete' type=\"button\" data-task-id=".concat(todo.taskId, ">X</button>\n            <span class='task ").concat(done, "' data-task-id=").concat(todo.taskId, ">").concat(todo.task, "</span>\n            </li>");
    }).join('');
    todolistEl.innerHTML = todosHtml;
  }

  function renderWelcome(tasksInfo) {
    document.querySelector('#todo-app .welcome span').innerText = "Welcome, ".concat(tasksInfo.username);
  }

  function renderErrorStatus(errs) {
    document.querySelector('#todo-app .error').innerHTML = errs.map(function (err) {
      return errMsgs[err.error] || err.error;
    });
  }

  function addAbilityToAdd() {
    document.querySelector('#todo-app .task-adder button').addEventListener('click', function (e) {
      var taskname = document.querySelector('#todo-app .task-adder input').value;
      (0,_services__WEBPACK_IMPORTED_MODULE_1__.addTask)(taskname).then(function (tasksInfo) {
  ...
  function addAbilityToDelete() {
    document.querySelector('#todo-app .todos').addEventListener('click', function (e) {
      if (!e.target.classList.contains('delete')) {
        return;
      }

      var taskId = e.target.dataset.taskId;
      (0,_services__WEBPACK_IMPORTED_MODULE_1__.deleteTask)(taskId).then(function (tasksInfo) {
        renderTodoList(tasksInfo);
      });
    });
  }
  ...
  function addAbilityToToggleDone() {
    document.querySelector('#todo-app .todos').addEventListener('click', function (e) {
      ...
      var taskId = e.target.dataset.taskId;
      (0,_services__WEBPACK_IMPORTED_MODULE_1__.toggleDone)(taskId).then(function (tasksInfo) {
        renderTodoList(tasksInfo);
      });
    });
  }
  ...
  function addAbilityToLogout() {
    document.querySelector('#todo-app .welcome button').addEventListener('click', function (e) {
      (0,_services__WEBPACK_IMPORTED_MODULE_1__.performLogout)().then(function () {
        return showLogin();
      });
    });
  }
  ...
  function updateTasksPeriodically() {
    (0,_services__WEBPACK_IMPORTED_MODULE_1__.getTasks)().then(function (tasksInfo) {
      renderTodoList(tasksInfo);
